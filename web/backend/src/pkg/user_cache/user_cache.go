...
func GetGroupListFromRedis(uId int32, rd redis.Conn) (*pb.GroupListRsp, error) {
	...
	log.Println("start get group list from redis")
	...
	for i, v := range groups {
		...
		log.Printf("Get Group info from cache: %+v", gInfo)
		if err != nil {
		...
		if err != nil {
			log.Printf("get user from group (%s) error: %s\n", string(gIds[i]), err)
			return nil, err
		}
...
func GetGroupMemDataFromCache(gid int32, rd redis.Conn) ([]*pb.UserRecord, error) {
	...
	log.Printf("group %d have user id : %+v in cache", gid, uids)

	sz := len(uids)
	if 0 == sz {
		log.Printf("group is not has any user\n")
		return nil, nil
	}
...
func UpdateUserFromDBToRedis(user *pb.UserRecord, v int) {
	...
	if err := AddUserDataInCache(&pb.Member{
		...
		log.Println("UpdateUserFromDBToRedis Add user information to cache with error: ", err)
	}
}
