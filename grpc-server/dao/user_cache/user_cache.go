...
func GetGroupMemDataFromCache(gid int32, rd redis.Conn) ([]*pb.UserRecord, error) {
	...
	for _, v := range uids {
		...
		if err != nil {
			log.Log.Info("get user online with err  with:%+v", err.Error())
			online = USER_OFFLINE
		}
...
func GetUserStatusFromCache(uId int32, redisCli redis.Conn) (int32, error) {
	...
	if value == nil {
		...
		if err != nil {
			log.Log.Println(">> get fail", err.Error())
			return USER_OFFLINE, err
		}
...
func GetUserFromCache(uId int32) (*pb.UserRecord, error) {
	...
	online, err := redis.Int(rd.Do("GET", MakeUserStatusKey(uId)))
	if err != nil {
		log.Log.Infof("get user online with err  with error: %+v", err.Error())
		online = USER_OFFLINE
	}
