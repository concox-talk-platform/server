...
func GetUserLocationInCache(uId int32, rd redis.Conn) (*pb.GPSHttpResp, *pb.GPS, error) {
	...
	log.Log.Printf("Get user %d  gps info : %v from cache", uId, resStr)
	if value[0] != nil { // 只要任意一个字段为空就是没有这个数据
		lon, err := strconv.ParseFloat(resStr[0], 128)
		if err != nil {
			log.Log.Printf("convent lon error:%v", err)
		}
		lat, err := strconv.ParseFloat(resStr[1], 128)
		if err != nil {
			log.Log.Printf("convent lat error:%v", err)
		}

		speed, err := strconv.ParseFloat(resStr[2], 64)
		if err != nil {
			log.Log.Printf("convent speed error:%v", err)
		}

		course, err := strconv.ParseFloat(resStr[3], 64)
		if err != nil {
			log.Log.Printf("convent course error:%v", err)
		}

		lTimeT, err := time.Parse(cfgComm.TimeLayout, resStr[4])
		if err != nil {
			log.Log.Printf("convent lTime error:%v", err)
		}
		gpsData = &pb.GPS{
