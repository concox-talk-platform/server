...
func AddAccount(a *model.CreateAccount) (int, error) {
	...
	if err != nil {
		log.Log.Println("get insert AddUser Fail")
		return -1, err
	}
...
func DeleteAccount(loginName string, pwd string) error {
	...
	if err != nil {
		log.Log.Printf("DeleteAccount error: %s", err)
		return err
	}
...
func GetAccount(key interface{}) (*model.Account, error) {
	...
	if err == sql.ErrNoRows {
		log.Log.Println("no rows")
		return nil, nil
	}

	// 赋值给返回的结构体
	//log.Log.Println("get account : ", id, "  ", username, " ", privilegeId, " ", pwd, " ", cTime)
	res := &model.Account{
		Id:          id,
		Pid:         pid,
		Username:    username,
...
func UpdateAccount(a *model.AccountUpdate) error {
	...
	if err != nil {
		log.Log.Println("update user error : ", err)
		return err
	}
	...
	if userRes != nil {
		...
		if err != nil {
			log.Log.Println("update user RowsAffected error : ", err)
			return err
		}
	}
	cusRes, err := tx.Exec(cusUpdStmt,
		...
		ctime, a.Id)
	if err != nil {
		log.Log.Println("update customer error : ", err)
		return err
	}
	...
	if cusRes != nil {
		...
		if err != nil {
			log.Log.Println("update customer RowsAffected error : ", err)
			return err
		}
	}

	log.Log.Println(userAff, "cusAff", cusAff)
